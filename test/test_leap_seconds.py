...
def test_leap_seconds():
    ...
    test_gps_leaps = ATM1b_QFIT.time.get_leap_seconds()
    assert all((v==t) for v,t in zip(valid_gps_leaps,test_gps_leaps))

#-- PURPOSE: update leap second file
def test_update_leap_seconds():
    ATM1b_QFIT.time.update_leap_seconds(verbose=False, mode=0o775)
    FILE = ATM1b_QFIT.utilities.get_data_path(['data','leap-seconds.list'])
